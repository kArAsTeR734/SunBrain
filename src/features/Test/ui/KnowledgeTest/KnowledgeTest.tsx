...
import { initialTasks } from '@shared/constants/tests.ts';
import TestResults from '@features/Test/ui/TestResut/TestResult.tsx';
import Task from '@entities/Task/ui/Task/Task.tsx';
import TestNavigation from '@features/Test/ui/TestNavigation/TestNavigation.tsx';

interface KnowledgeTestProps{
  have:boolean;
}

const KnowledgeTest: React.FC<KnowledgeTestProps> = ({have}) => {
  const [tasks] = useState(initialTasks);
  const [currentTaskIndex, setCurrentTaskIndex] = useState(0);
  const [userAnswers, setUserAnswers] = useState<Record<number, any>>({});
  const [showResults, setShowResults] = useState(have);
  ...
  return (
    <div className="math-test-container">
      <div className="test-header">
        ...
        <div className="timer">⏱️ Осталось времени: {formatTime(timeLeft)}</div>
      </div>
      ...
      <div className="test-progress">
        <div className="progress-bar">
          <div
            ...
            style={{
              width: `${(Object.keys(userAnswers).length / tasks.length) * 100}%`,
            }}
          />
        </div>
